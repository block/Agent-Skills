...
def validate(report):
    ...
    if not isinstance(unavailable, list) or not all(
        nonempty(item) for item in unavailable
    ):
    ...
    for index, source in enumerate(sources):
        ...
        if source.get("source_type") not in SOURCE_TYPES:
            errors.append(
                f"{label}.source_type must be primary, secondary, or aggregator"
            )
    ...
    if not isinstance(claims, list) or not claims:
        ...
        claims = []
    claim_ids, used_sources = set(), set()
    for index, claim in enumerate(claims):
        ...
        if not isinstance(refs, list) or not refs or not all(
            nonempty(item) for item in refs
        ):
        ...
        if not isinstance(count, int) or isinstance(count, bool) or count < 0:
            errors.append(
                f"{label}.independent_source_count must be a non-negative integer"
            )
        else:
            if count > len(set(refs)):
                errors.append(
                    f"{label} independent source count exceeds its source references"
                )
